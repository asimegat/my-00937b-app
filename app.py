...
# 1. 強化版資料抓取函數：使用網頁爬蟲繞過 API 限制
def get_stable_data():
    try:
        # 使用 Google Finance 抓取即時市價
        ticker = "00937B:TPE"
        url = f"https://www.google.com/finance/quote/{ticker}"
        response = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'})
        
        # 簡單爬蟲抓取價格 (尋找 class 包含YMlKec fxKbKc 的元素)
        from bs4 import BeautifulSoup
        soup = BeautifulSoup(response.text, 'html.parser')
        price_text = soup.find(class_='YMlKec fxKbKc').text
        current_price = float(price_text.replace('$', '').replace(',', ''))

        # 2. 正確的 00937B 配息數據清單 (截至 2025/12)
        # 考慮到 API 不穩定，直接將正確歷史數據寫入程式碼最保險
        div_data = {
            'Date': [
...
# 介面設計
st.title("🛡️ 00937B 投資成本監控 (穩定版)")

with st.sidebar:
    st.header("投資參數")
    buy_price = st.number_input("買進單價", value=15.54)
    shares = st.number_input("持有張數", value=10)
    buy_date = st.date_input("買進日期", value=datetime(2023, 12, 1))
...
if current_price:
    ...
    c1.metric("實質成本 (每股)", f"{real_cost:.3f}")
    c2.metric("累積領息", f"${dividend_received:,.0f}")
    c3.metric("總報酬率", f"{(total_gain/capital_in)*100:.2f}%")

    # 圓餅圖
    fig = go.Figure(data=[go.Pie(labels=['原始投入', '累積息收', '價差損益'], 
                                 values=[capital_in-dividend_received, dividend_received, max(0, market_value-capital_in)],
                                 hole=.4)])
    st.plotly_chart(fig)
    
    st.info(f"即時市價: {current_price} | 累計配息次數: {len(my_divs)} 次")
